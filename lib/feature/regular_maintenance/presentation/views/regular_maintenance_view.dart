...
void _confirmService() {
  ...
  String location;
  String? locationDetailsToSend; // لـ OrdersCubit

  if (_selectedLocationType == 'my_location') {
    location = _addressController.text.isNotEmpty
        ? 'Customer\'s Location: ${_addressController.text}'
        : 'Customer\'s Location (Will contact for details)';
    locationDetailsToSend = _addressController.text.isNotEmpty
        ? _addressController.text
        : 'Contact for details';
  } else if (_selectedLocationType == 'our_branches') {
    location = 'Branch: $_selectedBranch';
    locationDetailsToSend = _selectedBranch;
  } else {
    location = 'Location not specified';
    locationDetailsToSend = null;
  }
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        actions: <Widget>[
          ...
          ElevatedButton(
            onPressed: () async {
              // استدعاء OrdersCubit لحفظ الطلب
              context.read<OrdersCubit>().saveNewOrder(
                    selectedServices: _selectedServices.toList(), // تحويل الـ Set إلى List
                    selectedDate: _selectedDate!,
                    selectedTime: _selectedTime!,
                    locationType: _selectedLocationType!,
                    locationDetails: locationDetailsToSend,
                  );

              Navigator.of(context).pop(); // إغلاق مربع الحوار

              // يمكنك الاستماع لحالة OrdersCubit هنا لإظهار SnackBar
              // أو يمكنك تركه في OrderSavedSuccess state في OrdersCubit
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Your request is being processed...')),
              );
              // قد تحتاج للانتقال إلى شاشة الطلبات بعد الحفظ الناجح
            //  context.pushAndRemoveUntil(Routes.ordersView, predicate: (route) => false);
